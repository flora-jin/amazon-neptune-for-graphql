...
function graphDBInferenceSchema (graphdbSchema, addMutations, options = {}) {
    ...
    gdbs.nodeStructures.forEach(node => {
        ...
        r += `\t${inputQueryPrefix}get${nodeCase}(filter: ${nodeCase}Input): ${nodeCase}\n`;
        r += `\t${inputQueryPrefix}get${nodeCase}s(filter: ${nodeCase}Input, options: Options, sort: [${nodeCase}Sort!]): [${nodeCase}]\n`;
    });
    r += '}\n\n';
    ...
    if (addMutations) {
        ...
        gdbs.nodeStructures.forEach(node => {
            let nodeCase = toPascalCase(cleanseLabel(node.label));
            r += `\t${inputMutationPrefix}create${nodeCase}(input: ${nodeCase}CreateInput!): ${nodeCase}\n`;
            r += `\t${inputMutationPrefix}update${nodeCase}(input: ${nodeCase}UpdateInput!): ${nodeCase}\n`;
            r += `\t${inputMutationPrefix}delete${nodeCase}(_id: ID!): Boolean\n`;
        });    
        ...
        gdbs.edgeStructures.forEach(edge => {
            edge.directions.forEach(direction => {
                ...
                if (edge.properties.length > 0) {               
                    r += `\t${inputMutationPrefix}connect${fromCase}To${toCase}Through${edgeCase}(from_id: ID!, to_id: ID!, edge: ${edgeCase}Input!): ${edgeCase}\n`;
                    r += `\t${inputMutationPrefix}update${edgeCase}ConnectionFrom${fromCase}To${toCase}(from_id: ID!, to_id: ID!, edge: ${edgeCase}Input!): ${edgeCase}\n`;
                } else {
                    r += `\t${inputMutationPrefix}connect${fromCase}To${toCase}Through${edgeCase}(from_id: ID!, to_id: ID!): ${edgeCase}\n`;
                }
                r += `\t${inputMutationPrefix}delete${edgeCase}ConnectionFrom${fromCase}To${toCase}(from_id: ID!, to_id: ID!): Boolean\n`;
            });
        });
        r += '}\n\n';
    }
